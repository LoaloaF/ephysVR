...
def checkconfigs(path):
    pad_ids = [-1]
    colors = {}
    fnames = sorted(glob.glob(os.path.join(path, "*.cfg")))
    print("Found n configs: ", len(fnames))
    for f in fnames:
        pad_id = int(f[-8:-4])
    ...
    mapping = get_raw_implant_mapping(implant_name=implant_name).set_index("mea1k_el", drop=True)
    (fig, ax), el_rects = draw_mea1k(mapping=mapping, cmap_scaler=1)
