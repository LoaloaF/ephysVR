...
def main():
    ...
    PRVPATH = "/run/user/1000/gvfs/smb-share:server=yaniklab-data.local,share=large/BMI/VirtualReality/SpatialSequenceLearning/Simon/impedance/device_headmount_new2EpoxyWalls/impedance_bonded_dry_ext1KHz_rec4"
    # PATH = "/run/user/1000/gvfs/smb-share:server=yaniklab-data.local,share=large/BMI/VirtualReality/SpatialSequenceLearning/Simon/impedance/device_headmount_new3EpoxyWalls/impedance_bonded_extCurrent_singleElAll_2"
    PATH = "/run/user/1000/gvfs/smb-share:server=yaniklab-data.local,share=large/BMI/VirtualReality/SpatialSequenceLearning/Simon/impedance/device_headmount_new2EpoxyWalls/impedance_bonded_ext1KHz_Current_singelEl_rec1"
 

    el_connectivity_ranked = pd.read_pickle(PRVPATH+"/extracted_signal.pkl")#.index.values
    print(el_connectivity_ranked)
    el_connectivity_ranked = el_connectivity_ranked.sort_values('power')
    el_connectivity_ranked = el_connectivity_ranked.index.values
    print(el_connectivity_ranked)
    ...
    # els = [7786, 5798, 5981, 5761, 6869, 4011, 4900, 5120, 5318, 7315]
    # els = range(2190,26400)
    
    # for el_set_i in range(nsets):

    for i,target_el in enumerate(el_connectivity_ranked[::-1]):
        print(i, ": target_el ", target_el)

        covored_els = el_connectivity_ranked[:2000]
        # remove 16678 22178 22390
        covored_els = covored_els[(covored_els!=16678) & (covored_els!= 22178) & (covored_els!=22390) & (covored_els!=19352)]
        # covored_els = covored_els[covored_els>15060]
        print(covored_els.shape)

        config_set_els = np.append(covored_els, target_el)
        print(config_set_els)

        config_set_name = f"config_set_{target_el:05}"
        array = setup_array(config_set_els, stim_electrodes=None, config_name=config_set_name)
