...
def _sliding_window_parabolas(all_data, dac_range=100):
    ...
    for i in range(len(dac_vals)):
        # Include all points within dac_range/2 of the center point
        half_range = dac_range / 2
        mask = (dac_vals >= dac_vals[i] - half_range) & (dac_vals <= dac_vals[i] + half_range)
        ...
        # Score favors parabolas with the lowest vertex (c parameter)
        # We invert it (multiply by -1, or 1/c) so that sorting reverse=True picks the lowest point.
        # Adding R2 as a weak filter just to make sure it's an okay fit.
        score = (r2 / (popt[2] + 1.0)) if popt[2] >= 0 else 0

        window_results.append({
            'x': x, 'y': y, 'vertex': vertex, 'r2': r2, 
            'score': score, 'popt': popt, 'lowest_point': popt[2]
        })

    # Sort primarily by the lowest vertex point (ascending so the lowest comes first)
    # We filter only those with decent R2 first
    valid_windows = [w for w in window_results if w['r2'] > 0.3]
    if not valid_windows:
        valid_windows = window_results # Fallback if everything is bad

    valid_windows.sort(key=lambda d: d['lowest_point'])
    return valid_windows, (valid_windows[0] if valid_windows else None)
...
def _next_search_params(all_data, centered_around, delta, prev_vertices,
    ...
    2.  Sort the fitted parabolas by their lowest point (minimum vertex).
    ...
    if best is not None:
        ...
        print(f"  Best window: DAC {best['x'][0]:.0f}–{best['x'][-1]:.0f}  "
              ...
              f"lowest_point={best['lowest_point']:.1f}")
...
def _save_parabola_plot(all_data, set_id, window_results, best_window,
    ...
    for k, (wr, col) in enumerate(zip(window_results[:3], colors_w)):
        ...
        ls    = '-'  if k == 0 else '--'
        label = (f"Best (R²={wr['r2']:.2f}, lowest_point={wr['lowest_point']:.1f})"
...
def char_stim_units(dirname, n_amplifiers=2, stim_units=list(range(32)), debug=False):
    ...
    for stim_unit in stim_units:
        for which_amplifier in range(n_amplifiers):
            ...
            # RESET search parameters here so every amplifier starts with a full fresh sweep
            centered_around = 512
            delta = 512   # covers full [0, 1023] range initially
...
def eval_char_stim_units(dirname,  stim_units, debug=False):
    ...
    for stim_unit in stim_units:
        ...
        # final summary plot – overlay the parabola fit
        dac_vals_all = stimunit_res.DAC_val.values.astype(float)
        
        # Fit parabola on DAC values in range of 100 around the minimum
        mask_fit = (stimunit_res.DAC_val >= min_dac_value - 50) & (stimunit_res.DAC_val <= min_dac_value + 50)
        dac_vals_fit = stimunit_res.loc[mask_fit].DAC_val.values.astype(float)
        abs_peaks_fit = stimunit_res.loc[mask_fit].peak_uV.abs().values.astype(float)
        
        vertex_f, r2_f, popt_f = _fit_parabola(dac_vals_fit, abs_peaks_fit)
        ...
        if popt_f is not None:
            ...
            plt.plot(x_fit, _parabola(x_fit, *popt_f), color='crimson',
                     lw=1.5, ls='--', label=f'Local parabola fit (R²={r2_f:.3f})')
