...
def get_recording_config(path, fname):
    ...
    if mapping.shape[0] == 0:
        Logger().logger.error("Mapping is empty in h5, Infer mapping from path?")
        exit()
...
def _get_frame_nos(path, fname):
    ...
    if data_key is None:
        ...
        return np.arange(0, get_recording_shape(path, fname)[1])
    with h5py.File(os.path.join(path, fname), 'r') as file:
        frame_nos = np.array(file[data_key])
    return frame_nos

def get_recording_shape(path, fname):
    rec_file_fmt = _get_recording_version(path, fname)
