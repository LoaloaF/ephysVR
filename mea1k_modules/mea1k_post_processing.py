...
from mea1k_modules.mea1k_raw_preproc import get_recording_shape
from mea1k_modules.mea1k_raw_preproc import get_recording_config
...
def mea1k_raw2decompressed_dat_file(path, fname, session_name, animal_name, 
    ...
    try:
        _, rec_length = get_recording_shape(path, fname)
    except OSError as e:
        L.logger.error(f"Error: {e}")
        return
    chunk_indices = np.arange(0, rec_length, chunk_size_s*EC.SAMPLING_RATE)
...
def get_recording_implant_mapping(path, mea1k_rec_fname, animal_name=None,
    ...
    L = Logger()
    rec_config, _ = get_recording_config(path, mea1k_rec_fname)
...
def _make_neuroscope_xml(implant_mapping, out_fullfname):
    ...
    for channel_i, el_row in implant_mapping.iterrows():
        ...
        if el_row.shank_side == 'left':
            col = adjust_saturation(col, EC.METALLIZATION_COLOR_OFFSET)
...
#     implant_mapping = get_raw_implant_mapping(EC.NAS_DIR, EC.DEVICE_NAME)
